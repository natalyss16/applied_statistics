...
```{r}
ggplot(wine, aes(sample = analysed_variable, colour = as.factor(good))) +
  geom_qq() +
  geom_qq_line() +
  geom_abline(slope = 1, intercept = 0, color = "black") +
  scale_color_manual(values=c("#b37d69", "#6dcc6b"), labels=good_labels) +
  theme_minimal() +
  labs (
      x = "Theoretical quantiles",
      y = "Sample quantiles",
      title = "Sugar residuals QQ-plots",
      colour = "Wine quality"
    )
```

The distribution of both samples seems to be almost equal and right-skewed (this can also be seen on the histograms above).
